// ============================================================================
// QUEUEAPP - DISPLAY.JS (WITH ZONES + FOCUS MODE)
...
// ============================================================================
// CUSTOMER FOCUS MODE RENDERING
// ============================================================================

function renderFocusMode(rid, restaurant, customer, getZoneDisplay) {
  const isAllocated = customer.status === 'allocated';
  const bgColor = isAllocated ? '#22c55e' : '#f59e0b';
  const statusText = isAllocated ? 'SEATED' : 'WAITING';
  
  render(`
    <div style="min-height:100vh;background:${bgColor};padding:clamp(2rem,4vw,3rem)">
      <div style="max-width:800px;margin:0 auto">
        
        <div style="text-align:center;margin-bottom:2rem">
          <h1 style="color:white;font-size:clamp(2rem,6vw,4rem);margin-bottom:0.5rem">${restaurant.name}</h1>
          <div style="background:rgba(255,255,255,0.2);display:inline-block;padding:0.75rem 1.5rem;border-radius:999px;font-size:clamp(1rem,2.5vw,1.5rem);font-weight:700">
            ${statusText}
          </div>
        </div>
        
        ${isAllocated ? `
          <div style="background:white;padding:clamp(2rem,4vw,3rem);border-radius:1.5rem;text-align:center;box-shadow:0 8px 24px rgba(0,0,0,0.2)">
            <div style="font-size:clamp(3rem,8vw,5rem);margin-bottom:1rem">🎉</div>
            <h2 style="color:#22c55e;font-size:clamp(1.5rem,4vw,2.5rem);margin-bottom:1rem;font-weight:900">YOUR TABLE IS READY!</h2>
            
            ${getZoneDisplay(customer.zone) ? `
              <div style="background:#f59e0b;color:white;display:inline-block;padding:0.75rem 1.5rem;border-radius:999px;margin-bottom:1.5rem;font-weight:700;font-size:clamp(1rem,2.5vw,1.25rem)">
                ${getZoneDisplay(customer.zone)}
              </div>
            ` : ''}
            
            <div style="background:#f0fdf4;padding:clamp(1.5rem,3vw,2rem);border-radius:1rem;margin-bottom:1.5rem">
              <div style="font-size:clamp(0.875rem,2vw,1rem);color:#22c55e;margin-bottom:0.5rem">TABLE NUMBER</div>
              <div style="font-size:clamp(4rem,12vw,8rem);font-weight:900;color:#22c55e;line-height:1">${customer.tableNo}</div>
            </div>
            
            <div style="background:#f9fafb;padding:clamp(1.5rem,3vw,2rem);border-radius:1rem">
              <div style="font-size:clamp(0.875rem,2vw,1rem);color:#6b7280;margin-bottom:0.5rem">QUEUE NUMBER</div>
              <div style="font-size:clamp(2rem,6vw,4rem);font-weight:900;color:#1f2937;margin-bottom:1rem">${customer.queueNumber}</div>
              <div style="font-size:clamp(1.125rem,3vw,1.5rem);font-weight:600;color:#1f2937">${customer.name}</div>
              <div style="font-size:clamp(0.875rem,2vw,1rem);color:#6b7280;margin-top:0.5rem">👥 ${customer.guests} guest${customer.guests !== 1 ? 's' : ''}</div>
            </div>
          </div>
        ` : `
          <div style="background:white;padding:clamp(2rem,4vw,3rem);border-radius:1.5rem;text-align:center;box-shadow:0 8px 24px rgba(0,0,0,0.2)">
            <div style="font-size:clamp(3rem,8vw,5rem);margin-bottom:1rem">⏳</div>
            <h2 style="color:#f59e0b;font-size:clamp(1.5rem,4vw,2.5rem);margin-bottom:1rem;font-weight:900">PLEASE WAIT</h2>
            ...
            <div style="background:#fef3c7;padding:clamp(1.5rem,3vw,2rem);border-radius:1rem;margin-bottom:1.5rem">
              <div style="font-size:clamp(0.875rem,2vw,1rem);color:#92400e;margin-bottom:0.5rem">YOUR QUEUE NUMBER</div>
              <div style="font-size:clamp(4rem,12vw,8rem);font-weight:900;color:#f59e0b;line-height:1">${customer.queueNumber}</div>
            </div>
            
            <div style="background:#f9fafb;padding:clamp(1.5rem,3vw,2rem);border-radius:1rem">
              <div style="font-size:clamp(1.125rem,3vw,1.5rem);font-weight:600;color:#1f2937;margin-bottom:0.5rem">${customer.name}</div>
              <div style="font-size:clamp(0.875rem,2vw,1rem);color:#6b7280">👥 ${customer.guests} guest${customer.guests !== 1 ? 's' : ''}</div>
            </div>
          </div>
        `}
        
        <div style="text-align:center;margin-top:2rem">
          <button onclick="navigate('/r/${rid}/display')" class="btn" style="background:#64748b;color:white;font-size:clamp(0.875rem,2vw,1rem);padding:clamp(0.75rem,2vw,1rem) clamp(1.5rem,3vw,2rem);border:none;border-radius:0.5rem;cursor:pointer">
            📺 View Full Display
          </button>
        </div>
      </div>
...
// ============================================================================
// LIVE DISPLAY SCREEN WITH ZONES + FOCUS MODE
// ============================================================================
...
async function showDisplay(rid, customerQueueNumber) {
  ...
  window.displayUnsubscribe = db.collection('restaurants').doc(rid).onSnapshot(doc => {
    ...
    function getZoneDisplay(zoneId) {
      if (!zoneId || !zonesEnabled) return '';
      const zone = restaurant.zones.list.find(z => z.id === zoneId);
      return zone ? `${zone.emoji} ${zone.name}` : '';
    }
    
    // ✅ CUSTOMER FOCUS MODE
    if (customerQueueNumber) {
      const focusCustomer = restaurant.queue.find(q => q.queueNumber === customerQueueNumber);
      if (focusCustomer) {
        renderFocusMode(rid, restaurant, focusCustomer, getZoneDisplay);
        return;
      }
    }
    ...
    const displayWaitingQueue = waitingQueue;
      ...
      if (currentSettings.ui.cardStyle === 'simple') {
        return `
          <div class="card text-center" style="background:${isMyTurn ? 'linear-gradient(135deg,#fbbf24 0%,#f59e0b 100%)' : 'white'};padding:${cardPadding};border:4px solid ${isMyTurn ? '#f59e0b' : 'white'};${isMyTurn ? 'animation:pulse-badge 1s infinite;box-shadow:0 0 30px rgba(251,191,36,0.8)' : ''};min-height:280px;display:flex;flex-direction:column;justify-content:center">
            ...
            ${getZoneDisplay(allocated.zone) ? `
              <div style="background:#f59e0b;color:white;font-size:clamp(0.625rem,1.5vw,0.75rem);padding:0.25rem 0.5rem;border-radius:0.375rem;margin-bottom:0.5rem;font-weight:700;display:inline-block">
                ${getZoneDisplay(allocated.zone)}
              </div>
            ` : ''}
            ...
            <div style="max-height:60vh;overflow-y:auto">
              <div class="space-y">
                ${displayWaitingQueue.length > 0 ? displayWaitingQueue.slice(0, 15).map((w, i) => {
                    ...
                    <div class="card" style="background:${isTop3 ? 'rgba(249,115,22,.9)' : 'rgba(55,65,81,.8)'};padding:clamp(1rem,2vw,2rem);border:${isTop3 ? '3px solid var(--primary)' : '2px solid rgba(156,163,175,.3)'}">
                      <div style="display:flex;justify-content:space-between;align-items:center;gap:clamp(.75rem,1.5vw,1rem);flex-wrap:wrap">
                        <div style="flex:1">
                          ...
                          ${getZoneDisplay(w.zone) ? `
                            <div style="background:#f59e0b;color:white;font-size:clamp(0.75rem,1.75vw,0.875rem);padding:0.3rem 0.6rem;border-radius:0.4rem;margin-top:0.5rem;font-weight:700;display:inline-block">
                              ${getZoneDisplay(w.zone)}
                            </div>
                          ` : ''}
                          ${currentSettings.ui.showPhoneNumbers ? `
                            <div style="font-size:clamp(1rem,2vw,1.3rem);color:${isTop3 ? 'rgba(255,255,255,.8)' : 'rgba(255,255,255,.6)'};font-family:monospace;margin-top:.3rem">${w.phone}</div>
                          ` : ''}
                  ...
                  `;
                }).join('') : '<p class="text-center" style="color:rgba(255,255,255,.6);font-size:clamp(1.25rem,3vw,2rem);padding:2rem">No customers</p>'}
              </div>
            </div>
          </div>
        `;
      ...
      <div class="display-screen" style="position:relative">
        ...
        <div class="container">
          <div class="text-center mb" style="padding-bottom:clamp(1rem,2vw,2rem);border-bottom:4px solid var(--primary);position:relative">
            ...
            <h1 style="color:var(--primary);margin-top:clamp(2.5rem,5vw,4rem)">${restaurant.name}</h1>
...
console.log('✅ QueueApp Display Module Loaded (with Zones + Focus Mode)');
