...
function getZoneQRCode(rid,zoneId){return window.location.origin+"/#/r/"+rid+"/join?zone="+zoneId}
function generateZoneId(zoneName){return zoneName.toLowerCase().replace(/[^a-z0-9]/g,'').substring(0,20)}
function getZoneName(rid,zoneId){const restaurant=DB.restaurants[rid];if(!restaurant||!restaurant.zones||!restaurant.zones.enabled){return''}const zone=restaurant.zones.list.find(function(z){return z.id===zoneId});return zone?zone.name:''}
function getZone(rid,zoneId){const restaurant=DB.restaurants[rid];if(!restaurant||!restaurant.zones||!restaurant.zones.enabled){return null}return restaurant.zones.list.find(function(z){return z.id===zoneId})||null}
function isMultiZoneEnabled(restaurant){return restaurant.zones&&restaurant.zones.enabled===true}
function getMaxZonesForPlan(planPrice){planPrice=planPrice||1999;if(planPrice===1999)return 3;if(planPrice===2598)return 5;if(planPrice===2998)return 10;if(planPrice>=3000)return 999;return 3}
function getUpgradeMessage(planPrice){planPrice=planPrice||1999;if(planPrice===1999){return'Upgrade to ₹2,598/quarter for up to 5 zones (4-5 zones addon +₹599)'}if(planPrice===2598){return'Upgrade to ₹2,998/quarter for up to 10 zones (6-10 zones addon +₹999)'}if(planPrice===2998){return'Contact us for Enterprise plan (11+ zones) - Custom pricing'}return'Contact us for more zones'}
...
async function createDefaultGroundFloor(rid){const restaurant=DB.restaurants[rid];const defaultZone={id:'groundfloor',name:'Ground Floor',emoji:'🏢',scansToday:0,enabled:true,qrCode:getZoneQRCode(rid,'groundfloor'),createdAt:new Date().toISOString()};if(!restaurant.zones.list){restaurant.zones.list=[]}restaurant.zones.list.push(defaultZone);try{await db.collection('restaurants').doc(rid).update({zones:restaurant.zones})}catch(err){console.error('Error creating default zone:',err)}DB.restaurants[rid]=restaurant;DB.save();console.log('✅ Default Ground Floor zone created');return defaultZone}

// ============================================================================
// MULTI-ZONE UI GENERATOR (WITH DEFAULT GROUND FLOOR)
// ============================================================================

function generateMultiZoneUI(rid,restaurant){const isPremium=isPremiumActive(restaurant);const zonesEnabled=isMultiZoneEnabled(restaurant);if(!isPremium){return`<div style="background:linear-gradient(135deg,#fef3c7 0%,#fde68a 100%);padding:1.5rem;border-radius:1rem;border:3px dashed #f59e0b;position:relative;overflow:hidden;margin-top:1.5rem"><div style="position:absolute;top:0.75rem;right:0.75rem;background:#dc2626;color:white;padding:0.25rem 0.75rem;border-radius:999px;font-size:0.75rem;font-weight:700">🔒 PREMIUM</div><h4 style="margin:0 0 0.75rem 0;color:#92400e">🏢 Multi-Zone Queue Management</h4><p style="margin:0 0 1rem 0;font-size:0.875rem;color:#92400e">Manage multiple floors with separate QR codes</p><div style="opacity:0.4;filter:blur(3px);margin-bottom:1rem;pointer-events:none"><div style="background:white;padding:1rem;border-radius:0.5rem"><div style="display:flex;gap:0.5rem;flex-wrap:wrap"><div style="background:#f59e0b;color:white;padding:0.5rem 1rem;border-radius:0.5rem;font-size:0.75rem">🏢 Ground Floor</div><div style="background:#f59e0b;color:white;padding:0.5rem 1rem;border-radius:0.5rem;font-size:0.75rem">🏢 First Floor</div><div style="background:#f59e0b;color:white;padding:0.5rem 1rem;border-radius:0.5rem;font-size:0.75rem">🌳 Garden</div></div></div></div><button onclick="sessionStorage.setItem('upgrade_source','multizone_teaser');navigate('/pricing')" style="background:linear-gradient(135deg,#f97316 0%,#ec4899 100%);color:white;border:none;padding:1rem 1.5rem;border-radius:0.75rem;cursor:pointer;font-weight:700;width:100%;font-size:1rem;box-shadow:0 4px 12px rgba(249,115,22,0.4)">⚡ Unlock Multi-Zone (from ₹1,999/3mo)</button><p style="margin:0.75rem 0 0 0;text-align:center;font-size:0.75rem;color:#92400e">Includes: Unlimited customers • Auto cleanup • Analytics</p></div>`}if(!zonesEnabled){return`<div style="background:linear-gradient(135deg,#fef3c7 0%,#fde68a 100%);padding:1.5rem;border-radius:1rem;border:3px solid #f59e0b;margin-top:1.5rem"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem"><div><h4 style="margin:0;color:#92400e">🏢 Multi-Zone Management</h4><p style="margin:0.25rem 0 0 0;font-size:0.75rem;color:#92400e">✨ Premium Feature Available</p></div><span style="background:#22c55e;color:white;padding:0.25rem 0.75rem;border-radius:999px;font-size:0.75rem;font-weight:700">PREMIUM</span></div><p style="margin:0 0 1rem 0;font-size:0.875rem;color:#92400e">Create separate QR codes for different floors or zones (Ground Floor, First Floor, Garden, etc.)</p><button onclick="showZoneConfigModal('${rid}')" style="background:#f59e0b;color:white;border:none;padding:1rem 1.5rem;border-radius:0.75rem;cursor:pointer;font-weight:700;width:100%;font-size:1rem">⚙️ Configure Zones</button></div>`}const zones=restaurant.zones.list||[];const currentFilter=getZoneFilter();return`<div style="background:linear-gradient(135deg,#fef3c7 0%,#fde68a 100%);padding:1.5rem;border-radius:1rem;margin-top:1.5rem;border:3px solid #f59e0b"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem;flex-wrap:wrap;gap:0.5rem"><div><h4 style="margin:0;color:#92400e">🏢 Multi-Zone Management</h4><p style="margin:0.25rem 0 0 0;font-size:0.75rem;color:#92400e">✨ Premium Feature Active</p></div><button onclick="showZoneConfigModal('${rid}')" style="background:#f59e0b;color:white;border:none;padding:0.5rem 1rem;border-radius:0.5rem;cursor:pointer;font-weight:700;font-size:0.875rem">⚙️ Manage Zones</button></div><div style="background:white;padding:1rem;border-radius:0.75rem;margin-bottom:1rem"><p style="margin:0 0 0.75rem 0;font-weight:700;font-size:0.875rem;color:#4b5563">FILTER VIEW:</p><div style="display:flex;gap:0.5rem;flex-wrap:wrap">${zones.map(function(zone){return`<button onclick="filterZone('${rid}', '${zone.id}')" id="zone-btn-${zone.id}" class="zone-filter-btn" style="background:${currentFilter===zone.id?'#f59e0b':'#d1d5db'};color:${currentFilter===zone.id?'white':'#1f2937'};border:none;padding:0.5rem 1rem;border-radius:0.5rem;cursor:pointer;font-weight:700;font-size:0.75rem;transition:all 0.2s">${zone.emoji} ${zone.name} <span style="background:rgba(${currentFilter===zone.id?'255,255,255':'0,0,0'},0.2);padding:0.15rem 0.4rem;border-radius:999px;margin-left:0.25rem">${zone.scansToday||0}</span></button>`}).join('')}<button onclick="filterZone('${rid}', null)" id="zone-btn-all" class="zone-filter-btn" style="background:${!currentFilter?'#6b7280':'#d1d5db'};color:white;border:none;padding:0.5rem 1rem;border-radius:0.5rem;cursor:pointer;font-weight:700;font-size:0.75rem">🔄 All</button></div></div><div style="background:white;padding:1rem;border-radius:0.75rem;margin-top:1rem"><p style="margin:0 0 0.75rem 0;font-weight:700;font-size:0.875rem;color:#4b5563">Active Zones:</p><div style="display:flex;flex-direction:column;gap:0.5rem">${zones.map(function(zone){return`<div style="display:flex;justify-content:space-between;align-items:center;padding:0.75rem;background:#f9fafb;border-radius:0.5rem;border:2px solid ${currentFilter===zone.id?'#f59e0b':'#e5e7eb'}"><div style="display:flex;align-items:center;gap:0.5rem"><span style="font-size:1.25rem">${zone.emoji}</span><span style="font-weight:700;color:#1f2937">${zone.name}</span></div><span style="background:#e5e7eb;color:#6b7280;padding:0.25rem 0.75rem;border-radius:999px;font-size:0.75rem;font-weight:700">${zone.scansToday||0} today</span></div>`}).join('')}</div></div></div>`}

// ============================================================================
// ZONE FILTERING LOGIC
// ============================================================================

function filterZone(rid,zoneId){setZoneFilter(rid,zoneId);showRestaurantAdmin(rid)}
function applyZoneFilter(queue,zoneFilter){if(!zoneFilter){return queue}return queue.filter(function(item){return item.zone===zoneFilter})}

// ============================================================================
// ZONE CONFIGURATION MODAL (WITH AUTO-CREATE DEFAULT)
// ============================================================================
